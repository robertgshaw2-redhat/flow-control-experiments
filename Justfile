...
namespace   := env_var_or_default("NAMESPACE", "llm-test")
# Guide name; the EPP service is "<guide_name>-epp".
guide_name  := env_var_or_default("GUIDE_NAME", "qwen-basic")
# Derived EPP service name (the Service whose clusterIP we target).
epp_service := env_var_or_default("EPP_SERVICE", guide_name + "-epp-service")
# Model name as registered in vLLM (--served-model-name).
model       := env_var_or_default("MODEL_NAME", "Qwen/Qwen2.5-0.5B-Instruct")
# For RHAII: use gateway path instead of direct EPP
use_gateway := env_var_or_default("USE_GATEWAY", "true")
gateway_svc := "inference-gateway-istio.redhat-ods-applications.svc"
gateway_path := "/" + namespace + "/" + guide_name + "/v1/completions"
...
client *ARGS:
    ...
    kubectl run {{pod}} -n {{namespace}} --image={{image}} --restart=Never \
        --overrides='{"spec":{"serviceAccountName":"epp-client"}}' \
    ...
    # Run the client, injecting the gateway URL for RHAII
    GATEWAY_URL="http://inference-gateway-istio.redhat-ods-applications.svc/{{namespace}}/{{guide_name}}/v1/completions"
    kubectl exec -it {{pod}} -n {{namespace}} -- \
        env GATEWAY_URL="${GATEWAY_URL}" python3 /tmp/client.py --model {{model}} {{ARGS}}
...
flow-ui *ARGS:
    ...
    kubectl run {{pod}} -n {{namespace}} --image={{image}} --restart=Never \
        --overrides='{"spec":{"serviceAccountName":"epp-client","containers":[{"name":"{{pod}}","image":"{{image}}","command":["sleep","infinity"],"volumeMounts":[{"name":"scenarios","mountPath":"/tmp/scenarios"}]}],"volumes":[{"name":"scenarios","persistentVolumeClaim":{"claimName":"{{pvc_name}}"}}]}}' \
        --command -- sleep infinity
    kubectl wait --for=condition=Ready pod/{{pod}} -n {{namespace}} --timeout=120s
    ...
    kubectl exec -i {{pod}} -n {{namespace}} -- sh -c \
        'cd /tmp && nohup env GATEWAY_URL="'"${GATEWAY_URL}"'" python3 flow_ui_server.py --model {{model}} {{ARGS}} > /tmp/flow_ui.log 2>&1 &'
